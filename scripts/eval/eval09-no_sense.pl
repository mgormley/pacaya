...
print OUT "\n  SEMANTIC SCORES: \n";
my $apl = 0;
$apl = ($srl_counts{corl_arg}) * 100 / 
  ($srl_counts{pred_arg})
  if $srl_counts{pred_arg};
my $arl = 0;
$arl = ($srl_counts{corl_arg}) * 100 / 
  ($srl_counts{tot_arg})
  if $srl_counts{tot_arg};
my $afl = 0;
$afl = 2 * $apl * $arl / ($apl + $arl) if $apl + $arl;
printf OUT "  Labeled precision:          (%d) / (%d) * 100 = %.2f %%\n",
    $srl_counts{corl_arg}, 
    $srl_counts{pred_arg}, 
    $apl;
printf OUT "  Labeled recall:             (%d) / (%d) * 100 = %.2f %%\n",
    $srl_counts{corl_arg}, 
    $srl_counts{tot_arg}, 
    $arl;
...
my $apu = 0;
$apu = ($srl_counts{coru_arg}) * 100 / 
  ($srl_counts{pred_arg})
  if $srl_counts{pred_arg};
my $aru = 0;
$aru = ($srl_counts{coru_arg}) * 100 / 
  ($srl_counts{tot_arg})
  if $srl_counts{tot_arg};
my $afu = 0;
$afu = 2 * $apu * $aru / ($apu + $aru) if $apu + $aru;
printf OUT "  Unlabeled precision:        (%d) / (%d) * 100 = %.2f %%\n",
    $srl_counts{coru_arg}, 
    $srl_counts{pred_arg}, 
    $apu;
printf OUT "  Unlabeled recall:           (%d) / (%d) * 100 = %.2f %%\n",
    $srl_counts{coru_arg}, 
    $srl_counts{tot_arg}, 
    $aru;
