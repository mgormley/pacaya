...
public class DepParseEvaluatorTest {
    ...
    public void testUnicodePunctuationMatching() {
        ...
        assertTrue(DepParseEvaluator.PUNCT_RE.matcher(hebrewNunHafukha).matches());
        assertTrue(DepParseEvaluator.PUNCT_RE.matcher("-"+hebrewNunHafukha).matches());
        //assertTrue(DepParseEvaluator.PUNCT_RE.matcher("!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~").matches());
        assertTrue(DepParseEvaluator.PUNCT_RE.matcher("-"+hebrewNunHafukha+".!?,").matches());
        assertFalse(DepParseEvaluator.PUNCT_RE.matcher(",M.").matches());
        assertFalse(DepParseEvaluator.PUNCT_RE.matcher("012341").matches());
    }
