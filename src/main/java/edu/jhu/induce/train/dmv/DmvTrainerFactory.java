...
import edu.jhu.globalopt.BfsComparator;
import edu.jhu.globalopt.DfsNodeOrderer;
import edu.jhu.globalopt.NodeOrderer;
import edu.jhu.globalopt.PlungingBfsNodeOrderer;
import edu.jhu.globalopt.PqNodeOrderer;
import edu.jhu.globalopt.DfsNodeOrderer.DfsNodeOrdererPrm;
...
import edu.jhu.globalopt.PlungingBfsNodeOrderer.PlungingBfsNodeOrdererPrm;
import edu.jhu.globalopt.cpt.BasicCptBoundsDeltaFactory;
import edu.jhu.globalopt.cpt.CptBoundsDeltaFactory;
import edu.jhu.globalopt.cpt.FullStrongVariableSelector;
import edu.jhu.globalopt.cpt.MidpointVarSplitter;
import edu.jhu.globalopt.cpt.PseudocostVariableSelector;
...
import edu.jhu.globalopt.dmv.DmvSolutionEvaluator;
...
import edu.jhu.globalopt.dmv.ResDmvDantzigWolfeRelaxation.ResDmvDwRelaxPrm;
import edu.jhu.globalopt.dmv.ViterbiEmDmvProjector.ViterbiEmDmvProjectorPrm;
import edu.jhu.globalopt.dr.DimReducer.ConstraintConversion;
import edu.jhu.globalopt.dr.DimReducer.DimReducerPrm;
import edu.jhu.globalopt.dr.DimReducer.SamplingDistribution;
import edu.jhu.globalopt.randwalk.DfsRandChildAtDepthNodeOrderer;
import edu.jhu.globalopt.randwalk.DfsRandWalkNodeOrderer;
...
import edu.jhu.globalopt.rlt.Rlt.RltPrm;
import edu.jhu.globalopt.rlt.filter.MaxNumRltRowAdder;
import edu.jhu.globalopt.rlt.filter.RandPropRltRowAdder;
...
import edu.jhu.induce.model.dmv.FixableDmvModelFactory;
...
import edu.jhu.parse.ilp.IlpDepParser;
...
import edu.jhu.parse.ilp.InitializedIlpDepParserWithDeltas;
...
public class DmvTrainerFactory {
    ...
    private static DepParser getParser() throws ParseException {
        ...
        if (parserName.equals("cky")) {
            ...
            parser = new IlpDepSentenceParser(formulation, ilpSolverFactory);
        } else if (parserName.equals("ilp-corpus")) {
            parser = new IlpDepParser(formulation, ilpSolverFactory);
            ...
            if (parserName.equals("ilp-deltas")) {
                parser = new IlpDepParserWithDeltas(formulation, ilpSolverFactory, deltaGen);
            } else if (parserName.equals("ilp-deltas-init")) {
                parser = new InitializedIlpDepParserWithDeltas(formulation, ilpSolverFactory, deltaGen,
                        ilpSolverFactory);
            } else {
