...
public class Chart {
	...
	public Pair<CfgTree,Double> getViterbiParse() {
		CfgTreeNode root = getViterbiTree(0, sent.length, grammar.getRootSymbol());
		double rootScore = chart[0][sent.length].maxScores[grammar.getRootSymbol()];
		return new Pair<CfgTree, Double>(new CfgTree(root), rootScore);
	}
	 ...
	 * @return 
	 */
	private CfgTreeNode getViterbiTree(int start, int end, int rootSymbol) {
		ChartCell cell = chart[start][end];
		...
		CfgTreeNode leftChild;
		CfgTreeNode rightChild;
		if (bp.r.isLexical()) {
			leftChild = new CfgTreeNode(bp.r.getLeftChild(), start, end, null, null, true, grammar.getLexAlphabet());
			rightChild = null;
		...
		return new CfgTreeNode(rootSymbol, start, end, leftChild, rightChild, false, grammar.getNtAlphabet());
	}
}
