...
public class TagReducer {
    ...
    private static Logger log = Logger.getLogger(TagReducer.class);
    
    public static void reduceTags(DepTreebank trees){
        for (DepTree tree : trees) {
            reduceTags(tree);
        }
    }
    
    public static void reduceTags(DepTree tree){
        for (DepTreeNode node : tree) {
            if (node.getLabel() instanceof TaggedWord) {
                TaggedWord tw = (TaggedWord) node.getLabel();
                tw.setTag(reduceTag(tw.getTag()));
            }
        }
    }
     ...
     */
    public static String reduceTag(String tag) {
