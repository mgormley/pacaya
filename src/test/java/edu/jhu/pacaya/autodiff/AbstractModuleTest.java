...
public class AbstractModuleTest {
    ...
    private static final List<Algebra> test2Algebras = Lists.getList(RealAlgebra.SINGLETON, LogSignAlgebra.SINGLETON);
    private static final List<Algebra> test3Algebras = Lists.getList(RealAlgebra.SINGLETON, SplitAlgebra.SINGLETON, LogSignAlgebra.SINGLETON);
    ...
    public static void evalTensor1(Tensor t1, Tensor expT1Adj, Tensor1Factory fact, Tensor expOut, double adjFill) {
        evalTensor1(t1, expT1Adj, fact, expOut, adjFill, RealAlgebra.SINGLETON);
        evalTensor1(t1, expT1Adj, fact, expOut, adjFill, LogSignAlgebra.SINGLETON);
    }
    ...
    public static void evalTensor2(Tensor t1, Tensor expT1Adj, Tensor t2, Tensor expT2Adj, Tensor2Factory fact, Tensor expOut, double adjFill) {
        evalTensor2OneAlgebra(t1, expT1Adj, t2, expT2Adj, fact, expOut, adjFill, RealAlgebra.SINGLETON);
        evalTensor2OneAlgebra(t1, expT1Adj, t2, expT2Adj, fact, expOut, adjFill, LogSignAlgebra.SINGLETON);
    }
    ...
    public static void evalTensor1ByFiniteDiffs(Tensor1Factory fact) {
        Tensor t1 = TensorUtils.getVectorFromValues(RealAlgebra.SINGLETON, 2, 3, 5);
    ...
    private static <X extends MVec, Y extends MVec> void evalOneToOneByFiniteDiffsAbs(OneToOneFactory<X,Y> fact, Module<X> in1, VectorFactory vec) {        
        assert in1.getAlgebra().equals(RealAlgebra.SINGLETON);
        
        for (Algebra s : test2Algebras) {
            Module<X> in1Co = new ConvertAlgebra<X>(in1, s);
            Module<Y> main = fact.getModule(in1Co);
            Module<Y> mainCo = new ConvertAlgebra<Y>(main, RealAlgebra.SINGLETON);
            ...
            double delta = s.equals(SplitAlgebra.SINGLETON) ? 1e-2 : 1e-7;
    ...
    public static void evalTensor2ByFiniteDiffs(Tensor2Factory fact) {
        Tensor t1 = TensorUtils.getVectorFromValues(RealAlgebra.SINGLETON, 2, 3, 5);
        Tensor t2 = TensorUtils.getVectorFromValues(RealAlgebra.SINGLETON, 4, 6, 7);
        Identity<Tensor> in1 = new Identity<Tensor>(t1);
    ...
    public static <W extends MVec, X extends MVec, Y extends MVec> void evalTwoToOneByFiniteDiffs(
        ...
        assert in1.getAlgebra().equals(RealAlgebra.SINGLETON);
        assert in2.getAlgebra().equals(RealAlgebra.SINGLETON);
        ...
        for (Algebra s : test2Algebras) {
            ...
            IntDoubleVector x = vec.getVector(ModuleFn.getOutputSize(topo.getInputs()));
            double delta = s.equals(SplitAlgebra.SINGLETON) ? 1e-2 : 1e-8;
            ModuleTestUtils.assertGradientCorrectByFd(topo, x, 1e-5, delta);
        }
    }
